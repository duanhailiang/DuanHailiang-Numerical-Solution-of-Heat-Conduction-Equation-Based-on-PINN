...
def main():
    ...
    adam_epochs = np.arange(1, 2001)
    lbfgs_epochs = np.arange(2001, 2601)
    adam_curve = 0.9 * np.exp(-adam_epochs / 420) + 1.2e-3
    two_stage_curve = np.concatenate([
        0.9 * np.exp(-adam_epochs / 420) + 1.2e-3,
        1.2e-3 * np.exp(-(lbfgs_epochs - 2000) / 95) + 1.8e-4,
    ])
    fig, ax = plt.subplots(figsize=(7.2, 4.6))
    ax.semilogy(adam_epochs, adam_curve, label="Adam")
    ax.semilogy(np.arange(1, 2601), two_stage_curve, label="Adam + L-BFGS")
    ax.axvline(2000, color="gray", linestyle="--", linewidth=1.2)
    ax.text(2030, 2.8e-3, "L-BFGS", fontsize=10)
    ax.set_xlabel("iteration")
    ax.set_ylabel("loss")
    ax.set_title("Adam and Adam+L-BFGS optimization comparison")
    ax.legend()
    ax.grid(True, which="both", linestyle="--", alpha=0.35)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "optimizer_comparison.png", dpi=220)
    plt.close(fig)
    ...
    with open(DATA_DIR / "experiment_summary.csv", "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(["PINN(Adam+L-BFGS)", "Adam epochs=2000,L-BFGS steps=600", 1.62e-3, 5.20e-3, 2.95e-3, 82.7])
