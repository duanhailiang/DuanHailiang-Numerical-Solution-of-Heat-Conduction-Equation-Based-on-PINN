...
def compute_loss(model, points, alpha):
    x_f, t_f, x_ic, t_ic, u_ic, x_bc, t_bc, u_bc = points
    r = pde_residual(model, x_f, t_f, alpha)
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        points = sample_points(args.n_f, args.n_ic, args.n_bc, device)
        loss, loss_pde, loss_ic, loss_bc = compute_loss(model, points, args.alpha)
    ...
    rmse, linf, rel_l2 = evaluate(model, args, device)
    print(f"After Adam: RMSE={rmse:.6e}, L_inf={linf:.6e}, relative_L2={rel_l2:.6e}")

    if args.lbfgs_steps > 0:
        fixed_points = sample_points(args.n_f, args.n_ic, args.n_bc, device)
        ...
        def closure():
            lbfgs.zero_grad()
            loss, _, _, _ = compute_loss(model, fixed_points, args.alpha)
            loss.backward()
            return loss

        print(f"Running L-BFGS refinement for at most {args.lbfgs_steps} steps...")
        lbfgs.step(closure)
        loss, loss_pde, loss_ic, loss_bc = compute_loss(model, fixed_points, args.alpha)
        history.append(
            [
        ...
        rmse, linf, rel_l2 = evaluate(model, args, device)
        print(f"After Adam+L-BFGS: RMSE={rmse:.6e}, L_inf={linf:.6e}, relative_L2={rel_l2:.6e}")
    ...
    np.savetxt(out / "metrics.csv", np.array([[rmse, linf, rel_l2]]), delimiter=",",
               header="rmse,linf_error,relative_l2", comments="")
